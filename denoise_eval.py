...
def find_good_mu(noise_mag):
    """
    Computes the regularization parameter for adversarial regularizer according heuristic in Lunz et al. (2018)
    """ 
...
def denoise_eval(opts):
    ...
    denoising_eval_folder = 'denoising_evaluation'
    if denoising_eval_folder in os.listdir(folder):
        i = 1
        while f'past_denoising_evaluation_{i}' in os.listdir(folder):
            i += 1
        shutil.move(os.path.join(folder, denoising_eval_folder), os.path.join(folder, f'past_denoising_evaluation_{i}'))
    denoising_eval_folder = os.path.join(folder, denoising_eval_folder)
    ...
    ttc_denoised_batches, _ = propagator.propagate_and_keep(noisy_batch.detach().clone())
    ...
    if opts.benchmark:
        ...
        clean_batch_ = clean_batch_.to(device[0])
    ...
    for i, ttc_denoised_batch in enumerate(ttc_denoised_batches):
        ...
        generate_samples.batch_comparisons(step_images_folder, batches_to_save, grid_columns=grid_columns, ext='pdf', grid_dpi=3000)
