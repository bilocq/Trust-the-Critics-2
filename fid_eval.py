...
def fid_eval(opts):
    ...
    for i in tqdm(range(num_batch)):
        ...
        save_source = ((0,0) not in results_df.index)
    ...
    step_folders = [f for f in os.listdir(gen_data_folder) if f[0:4]=='step'] # Make FID evaluations in order
    step_folders = [sf for _, sf in sorted(zip([int(sf.split('_')[1]) for sf in step_folders], step_folders))]
    if 'source' in os.listdir(gen_data_folder):
        step_folders.insert(0, 'source')
    start_time = time.perf_counter()
    ...
    # Remove temp folder with generated data and version of target saved for fid evaluation
    if compute_target_stats:
        shutil.rmtree(target_)
    if not opts.keep_gen:
        shutil.rmtree(gen_data_folder)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--keep_gen',   action='store_true',       help="If True, data generated for FID evaluation is kept, otherwise it is deleted at the end.")
    
    # FID evaluation settings
    parser.add_argument('--eval_steps', default='all_steps', choices=['all_steps', 'all_critics', 'last_step'], help="Specifies after which TTC steps to evaluate FID.")
