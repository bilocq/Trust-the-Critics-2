...
def train(opts):
    ...
    for i in range(num_pretrained_crit, num_crit):
        ...
        for j, th in enumerate(propagator.step_dict['theta'][i]):
            wasserstein_d = ttc_tools.get_wasserstein_D(critic, 100, source_loader, target_loader, propagator, loss_params)
    ...
    if opts.generate_images:
        ...
        if batch.size(0) > 64:
            batch = batch[:64]
        propagator.propagate_and_save(images_path, 'batch_image', batch, batch_image=True, save_source=True)
...
if __name__ == '__main__':
    ...
    # Model settings
    parser.add_argument('--model',    default='infogan', choices=['dcgan', 'infogan', 'arConvNet', 'sndcgan','bsndcgan'], help="Architecture for critics to be trained")
