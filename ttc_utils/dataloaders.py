...
def dataloader_tool(data, bs, shape=[None,None,None], pretransforms=None, device=None, train=True, random_crop=False, noise_bound=-1., num_workers=0):
    """
    ...
    Note: 'device' argument not used in this version.
    """    
    ...
    # Standard Gaussian noise loader
    if data=='noise':
        ...
        if noise_bound==-1: # No truncation
            return NoiseLoader(bs, shape)
        else: 
            assert noise_bound > 0, "Noise bound for truncation must be strictly positive."
            return TruncNoiseLoader(bs, shape, noise_bound)
    ...
    if 'MNIST' in os.listdir(data):            
        return mnist(data, bs, shape, pretransforms, train, num_workers)
    
    # Is Fashion-MNIST here?
    if 'FashionMNIST' in os.listdir(data): 
        return fashion(data, bs, shape, pretransforms, train, num_workers)
    
    # Otherwise, assume ImageFolder format
    else:
        return image_folder(data, bs, shape, pretransforms, train, random_crop, num_workers)
     
    

class NoiseLoader:
    def __init__(self, bs, shape):
        self.bs = bs
        self.shape = shape
...
class TruncNoiseLoader:
    def __init__(self, bs, shape, noise_bound):
        self.bs = bs
        self.c, self.h, self.w = shape
        self.num_samp = bs * self.c * self.h * self.w
        self.bound = noise_bound
        self.dataset_length = float('inf')

    def __iter__(self):
        return self

    def __next__(self):
        output = self.truncated_gaussian().view(self.bs, self.c, self.h, self.w)
    ...
    def truncated_gaussian(self):
        ...
        this number in absolute value will be resampled. 
        Output
        ...
        count = 0
        num_per_batch = self.num_samp # number of points to generate each iteration
    
        while samples.size(0) < self.num_samp:
            x = torch.randn(num_per_batch)
            valid = (x > -self.bound) & (x < self.bound)
            samples = torch.cat((samples, x[valid]), dim = 0)
            count += 1
            num_per_batch = max(self.num_samp - samples.size(0), 100000)
        samples = samples[0:self.num_samp]
        return samples



def mnist(data, bs, shape, pretransforms, train, num_workers=0):
    # Preprocessing
    transforms_ = []
...
def fashion(data, bs, shape, pretransforms, train, num_workers=0):
    # Preprocessing
    transforms_ = []
    height = 28 if (shape[1] is None) else shape[1]
    width  = 28 if (shape[2] is None) else shape[2]
...
def image_folder(data, bs, shape, pretransforms, train, random_crop=False, num_workers=0):      
