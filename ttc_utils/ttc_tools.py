        ...
        for crit_idx, critic in enumerate(self.trained_critics):
            ...
            for st, ty, keep_step in zip(self.step_dict['step_size'][crit_idx],
                ...
                if keep_step:
                    ...
                    step_inds.append(step_ind)
        return returned_steps, step_inds
        
    def propagate_and_save(self, save_folder, key, batch, batch_idx=None, batch_image=False, save_source=True, to_rgb=False, ext='jpg', grid_columns=16):
        """
...
# ---------------------------------------------------------------------------------------------
"""
Functions for dealing with TTC step_dicts and the 'step_info' inputs in train.py and FID_eval.py.
"""
...
def unpack_step_block(sb, default_step_params=None): # sb for 'step block'
    """
    Used by 'unpack_step_info'.
    """
    re_pattern = re.compile("^[0-9]*\[[0-9a-z,.]+\]$")  # Regex pattern for valid step blocks that aren't just a number of steps.
    assert (re_pattern.match(sb) or (sb.isdigit() and default_step_params is not None)), f"Step block {sb} is invalid."
    if sb.isdigit():
        sb = sb + str(default_step_params).replace(' ', '')       
    ...
    for i, param in enumerate(sb[sb.find('[')+1:-1].split(',')):
        ...
        try:
            # Parameters specified as an int or float with no prefix is assumed to be in the correct position 
...
def unpack_step_info(step_info, default_step_params=None):
    """
    ...
    specify how many critics should be trained, how many TTC steps should be taken on each critic, as well as the step type and
    theta parameter for each step. Note that all experiments reported in the paper used step_type=1 and theta=1.
    
        - The 'step_info' argument should be a string containing one or more "step_patterns" separated (if there is more that one) by
          '__:__'. 
        ...
        - Each step block should have the form n[ty,th] where:
                n is an integer specifying how many steps with these parameters to take (if omitted, then n=1 is filled in),
          ...
          Alternatively, a step block can consist of a single integer (e.g. '3' or '12'), if the 'default_step_params' argument is
          specified as a list of two elements, in which case the [ty,th] part of the step block is automatically filled in.
    
    EXAMPLES
        - step_info = '(20)_1[1,1]' OR step_info = '(20)_1': 
        ...
        - step_info = '(10)_2[1,1]__:__(20)_1[1,1]' :
            10 critics on each of which 2 steps will be taken followed by 20 critics on each of which 1 step will be taken 
            (all steps are of type 1 with theta=1)
        - step_info = '(3)_2[1,1]_1[2,0.9]__:__(4)_5[1,0.7]' :
            2 critics on each of which will be taken 2 steps of type 1 with theta=1 followed by 1 step of type 2 with theta=0.9,
            then 4 critics on which will be taken 5 steps of type 1 with theta=0.7
        - step_info = '(5)_2__:__(10)_[2,0.7]' AND default_step_params=[1,1]:
            5 critics on each of which 2 steps of type 1 with theta=1 will be taken, then 10 critics on each of which 1 step of
            type 2 with theta=0.7 will be taken.
    """
    step_dict = {'type':  [], 
                 'theta': []}
